...
struct P
{
    ...
    db alpha() { return atan2(y, x); }  // [-π,π]
    void read() { cin >> x >> y; }
    void write() { cout << "(" << x << "," << y << ")" << endl; }
    db abs() { return sqrt(abs2()); }
    db abs2() { return x * x + y * y; }
    P rot90() { return P(-y, x); }
    P unit() { return *this / abs(); }  // 返回单位向量
...
// 大于0, 说明从p1视角来看, p3在p2的逆时针方向, 等于0, 说明三点共线, 小于0说明从p1视角来看, p3在p2的顺时针方向
#define crossOp(p1, p2, p3) sign(cross(p1, p2, p3))
// 判断直线p1p2, q1q2是否恰有一个交点
...
// 判断线段p1p2, q1q2是否严格相交, 不能交在端点, 不能有公共的部分
...
// 判断点q是否严格在线段p1p2上(不包含端点)
bool onSeg_strict(P p1, P p2, P q)
{
    return crossOp(p1, p2, q) == 0 && sign((q - p1).dot(p1 - p2)) * sign((q - p2).dot(p1 - p2)) < 0;
}
// 求q到直线p1p2的投影(垂足), 注意:p1不能等于p2
...
// 求线段p1p2与线段q1q2之间的距离(在p1p2里找一个点, q1q2里找一个点, 满足这两个点距离最小)
...
vector<P> convexHull(vector<P> ps)
{
    ...
    for (int i = n - 2, t = k; i >= 0; qs[k ++ ] = ps[i -- ])
        while (k > t && crossOp(qs[k - 2], qs[k - 1], ps[i]) <= 0)
            k -- ;
    qs.resize(k - 1); // 0号点加了两次, 将多加的删去
    return qs;
}
// 求非严格的凸包, 共线的算进去
vector<P> convexHullNonStrict(vector<P> ps)
{
    // caution: need to unique the Ps first  给所有点去重
    int n = ps.size();
    if (n <= 1)
        return ps;
    sort(ps.begin(), ps.end());
